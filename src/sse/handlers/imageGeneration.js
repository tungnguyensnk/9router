...
export async function handleImageGeneration(request) {
  ...
  if (comboData) {
    ...
    log.info("IMAGE", `Combo "${modelStr}" with ${comboData.models.length} models (strategy: ${comboStrategy}, sticky: ${comboStickyLimit})`);
    return handleComboChat({
      body,
      models: comboData.models,
      handleSingleModel: (b, m) => handleSingleModelImage(b, m, { wantsStream, binaryOutput, preferredConnectionId, allowedConnectionIds: comboData.accountFilters?.[m] || [] }),
      log,
      comboName: modelStr,
      comboStrategy,
      comboStickyLimit,
