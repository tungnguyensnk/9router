...
export async function handleTts(request) {
  ...
  if (comboData) {
    ...
    log.info("TTS", `Combo "${modelStr}" with ${comboData.models.length} models (strategy: ${comboStrategy}, sticky: ${comboStickyLimit})`);
    return handleComboChat({
      body,
      models: comboData.models,
      handleSingleModel: (b, m) => handleSingleModelTts(b, m, responseFormat, language, { allowedConnectionIds: comboData.accountFilters?.[m] || [] }),
      log,
      comboName: modelStr,
      comboStrategy,
      comboStickyLimit,
